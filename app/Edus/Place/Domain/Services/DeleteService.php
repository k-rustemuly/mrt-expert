...
class DeleteService extends Service
{
    ...
    public function handle($place_id = 0)
    {
        ...
        // Удаляем информацию об одном месте 
        if($this->repository->deleteWhere($where) > 0) 
        {
            return new SuccessPayload(__("Success deleted"));
        }
        throw new MainException("Not deleted");        
    }
